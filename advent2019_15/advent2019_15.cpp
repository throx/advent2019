...
void PrintXY(const Point2& pos, CPU::MemType val)
{
    if (pos[0] < XOFFSET && pos[0] > -XOFFSET && pos[1] < YOFFSET && pos[1] > -YOFFSET) {
        gotoxy(pos[0] + XOFFSET, pos[1] + YOFFSET);
        Print(val);
    }
}

void Update(Map& map, const Point2& pos, CPU::MemType val)
{
    map[pos] = val;
    PrintXY(pos, val);
}
...
int g_move = 0;
CPU::MemType g_moves[4] = { 1, 3, 2, 4 };
Point2 g_p[4] = { Point2{0, -1}, Point2{-1, 0}, Point2{0, 1}, Point2{1, 0} };
int g_index = 0;
Point2 g_target;
int g_state = 0;

CPU::MemType GetMove(Map& map, const Point2& pos)
{
    return g_moves[g_move];
}

void SetResult(Map& map, Point2& pos, CPU::MemType result)
{
    Point2 newpos = pos + g_p[g_move];
    switch (result) {
    case 0:
        Update(map, newpos, L_WALL); // wall
        --g_move;
        break;

    case 1:
        Update(map, newpos, L_HALL); // hall
        ++g_move;
        pos = newpos;
        break;

    case 2:
        Update(map, newpos, L_TARGET); // target
        ++g_move;
        pos = newpos;
        g_target = pos;
        break;
...
int FloodFillDist(const Map& map, const Point2& start, const Point2& finish)
{
    Map ffdist;
    ffdist[start] = 0;
    CPU::MemType dist = 0;
    ...
    while (!finished) {
        ...
        for (auto& p : ffdist) {
            if (p.second == dist) {
                for (int i = 0; i < 4; ++i) {
                    Point2 np = p.first + g_p[i];
                    if (ffdist.find(np) == ffdist.end()) {
                        auto it = map.find(np);
                        if (it != map.end() && it->second != L_WALL) {
                            ffdist[np] = dist + 1;
                            finished = false;
                        }
                    }
    ...
    return ffdist[finish];
}

// I may have used copy-paste here...
int FloodFillMax(const Map& map, const Point2& start)
{
    Map ffdist;
    ffdist[start] = 0;
    CPU::MemType dist = 0;
...
int main()
{
    ...
    cpu.SetIO(
        [&]() { return GetMove(map, pos); },
        [&](CPU::MemType val) {
            SetResult(map, pos, val);
        });
    Load(cpu.GetRam());
    while (cpu.ExecuteNext() && g_state == 0)
        ;

    // Do floodfill
    int dist = FloodFillDist(map, Point2(), g_target);
    gotoxy(0, YOFFSET * 2 + 1);

    cout << fg_white << "Distance = " << dist << endl;

    // Work out o2 max dist
    dist = FloodFillMax(map, g_target);

    gotoxy(0, YOFFSET * 2 + 2);

    cout << fg_white << "Max = " << dist << endl;
    return 0;
}
